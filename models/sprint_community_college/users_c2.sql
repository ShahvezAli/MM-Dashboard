with source_data as (
    ...
    {{ source('public_sprint_community_college_data_set', 'mm_users')}} users
    inner join {{ source('public_sprint_community_college_data_set', 'mm_colleges')}} college
    on college.id =  users.college_id
    where college.college_name = 'Sprint Community College'
)
