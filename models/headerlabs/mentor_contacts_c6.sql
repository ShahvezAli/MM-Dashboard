with source_data as (
    ...
    from {{ source('headerlabs_data_set', 'mentors_c6')}} users
    inner join {{ source('headerlabs_data_set', 'friends_c6')}} contacts
...
friend_source_data as (
    ...
    from source_data
    inner join {{ source('headerlabs_data_set', 'users_c6')}} friends
