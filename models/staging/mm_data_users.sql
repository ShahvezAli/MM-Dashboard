with mentee_users as (
   select 
	...
	count(1) filter (where (user_type = 'Mentee' and date(updated_at) > NOW() - INTERVAL '1' MONTH)) as last_month_active_mentee,
        ...
        count(1) filter (where (is_profile_complete = 'false')) as total_incomplete_profile,
   ...
   from {{ source('dashboard_data_set', 'mm_users')}}
),
final as (
    select
	total_user_count,
	...
	total_incomplete_profile,
	total_incomplete_profile_mentee,
        ((total_incomplete_profile::numeric(16, 2)*100)/total_user_count)::numeric(16, 2) as incomplete_profile_percent,
