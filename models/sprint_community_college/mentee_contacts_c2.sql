with source_data as (
    ...
    from {{ source('sprint_community_college_data_set', 'mentees_c2')}} users
    inner join {{ source('sprint_community_college_data_set', 'friends_c2')}} contacts
...
friend_source_data as (
    ...
    from source_data
    inner join {{ source('sprint_community_college_data_set', 'users_c2')}} friends
