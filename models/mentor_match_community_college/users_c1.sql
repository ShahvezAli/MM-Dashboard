with source_data as (
    ...
    {{ source('public_mentor_match_community_college_data_set', 'mm_users')}} users
    inner join {{ source('public_mentor_match_community_college_data_set', 'mm_colleges')}} college
    on college.id =  users.college_id
    where college.college_name = 'Mentor Match Community College'
)
